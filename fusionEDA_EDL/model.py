...
class FusionMLPPredictor(nn.Module):
    def __init__(self, h_dim, hidden_dim, dropout, eval_dd_etypes, G, proto, proto_num, sim_measure, bert_measure, agg_measure, num_walks, walk_mode, path_length, data_folder, exp_lambda, device):
        ...
        if proto:
            ...
            if sim_measure == 'llm':
                ...
                disease_etypes = ['rev_GEN_DIS_ass','rev_MF_DIS_ass','rev_BP_DIS_ass','rev_CC_DIS_ass','rev_PWY_DIS_ass']
                disease_nodes = ['gene/protein','molecular_function','biological_process','cellular_component','pathway']
    ...
    def apply_edges(self, edges):
        ...
        predict = self.mlp(x)
        out = nn.functional.softplus(predict) + 1
        return {"score": out}
