...
from .EDAData import EDAData, TokenEmbLoad,ShardedTokenStore
from .EDAData import extract_edges_from_graph,EdgePairDataset,token_collate_fn
from .EDAEval import EDAEval
...
class EDAGNN:  
    def __init__(self, data,
        ...
        self.pwy_path = pwy_path
        self.bp_path = bp_path
        self.mf_path = mf_path
        self.cc_path = cc_path
        
        self.dd_etypes = [('compound', 'CPD_DIS_ass', 'disease'), 
                        ('disease', 'rev_CPD_DIS_ass', 'compound')]
        self.eval_dd_etypes = self.dd_etypes
    ...
    def model_initialize(self, n_hid = 128, 
        ...
        self.G = initialize_node_embedding(self.G, n_inp, 
                                            ...
                                            dis_path=self.dis_path,
                                            gen_path=self.gen_path,
                                            pwy_path=self.pwy_path,
        ...
        self.g_train_pos, self.g_train_neg, self.g_valid_pos, self.g_valid_neg,self.g_test_pos, self.g_test_neg = graphs_construct(data_path= self.data_path,device = self.device)
        ...
        self.model = HeteroRGCN(self.G,
                                ...
                                device = self.device,
                                eval_dd_etypes = self.eval_dd_etypes
                                ).to(self.device)    
        self.best_model = self.model
        
    def pretrain(self, n_epoch = 1, learning_rate = 1e-3, batch_size = 1024, train_print_per_n = 20, sweep_wandb = None,save_name = None):        
        if self.no_kg:
        ...
        wrapped_sampler = dgl.dataloading.EdgePredictionSampler(
            sampler,
            negative_sampler=Minibatch_NegSampler(self.G, 1, 'fix_dst'),
        )
        dataloader = dgl.dataloading.DataLoader(
            self.G, train_eid_dict, wrapped_sampler,
        ...
        save_pretrain = os.path.join(save_name,'pretrain_md')
        os.makedirs(save_pretrain,exist_ok = True)
        if self.config is None:
            raise ValueError('No model is initialized...')
        with open(os.path.join(save_pretrain, 'config.pkl'), 'wb') as f:
            pickle.dump(self.config, f)
        torch.save(self.model.state_dict(), os.path.join(save_pretrain, 'model.pt'))        
        self.best_model = copy.deepcopy(self.model)

    def finetune(self, pkl_dir = None,
                       n_epoch = 500, 
                       learning_rate = 1e-3, 
                       train_print_per_n = 5, 
                       valid_per_n = 25,
        ...
        if freeze_mode == "mlp_only":
            for p in self.model.parameters():
                p.requires_grad = False
            for p in self.model.mlp_pred.parameters():
                p.requires_grad = True
            optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad, self.model.parameters()),lr=learning_rate,weight_decay=1e-4)
        elif freeze_mode == "layer2_mlp":
            for p in self.model.parameters():
                p.requires_grad = False
            ...
            for p in self.model.mlp_pred.parameters():
                p.requires_grad = True
            optimizer = torch.optim.AdamW([{"params": self.model.layer2.parameters(), "lr": learning_rate * 0.1},
                                            {"params": self.model.mlp_pred.parameters(), "lr": learning_rate},],weight_decay=1e-4)
        elif freeze_mode == "none":
            optimizer = torch.optim.AdamW(self.model.parameters(), lr = learning_rate)
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', 0.8) 

        for epoch in range(n_epoch):
            pred_score_pos, pred_score_neg, pos_score, neg_score = self.model(self.G, self.g_train_neg, mode = 'train', pretrain_mode = False)
            pos_score = torch.cat([pred_score_pos[i] for i in self.dd_etypes]) 
            neg_score = torch.cat([pred_score_neg[i] for i in self.dd_etypes]) 

            alphas = torch.cat([pos_score, neg_score],dim=0)
            labels = torch.cat([torch.ones(pos_score.shape[0], device=self.device),torch.zeros(neg_score.shape[0], device=self.device),]).float()
            loss = dirichlet_loss(labels, alphas=alphas,device=self.device, lam=0.05)

            prob_pos, _, _, _, _ = predict_from_alpha(alphas)
            scores = prob_pos.detach().cpu().numpy()
            ...
            if epoch % train_print_per_n == 0:
                # training tracking...
                auroc_rel, auprc_rel, micro_auroc, micro_auprc, macro_auroc, macro_auprc = get_all_metrics_fb_evi(pred_score_pos, pred_score_neg, scores, labels_np, self.G, True)
            ...
            if (epoch) % valid_per_n == 0:
                # validation tracking...
                print('Validation.....')
                auroc_rel, auprc_rel, micro_auroc, micro_auprc, macro_auroc, macro_auprc,acc_rel,se_rel,spe_rel,pre_rel,f1_rel,loss = evaluate_evi(self.model, self.g_valid_pos, self.g_valid_neg, self.G, self.eval_dd_etypes, self.device, mode = 'valid', return_raw=False)
        ...
        if self.config is None:
            raise ValueError('No model is initialized...')     
        with open(os.path.join(save_name, 'config.pkl'), 'wb') as f:
            pickle.dump(self.config, f)
        torch.save(self.best_model.state_dict(), os.path.join(save_name, 'model.pt'))

        print('Testing.......')
        auroc_rel, auprc_rel, micro_auroc, micro_auprc, macro_auroc, macro_auprc,acc_rel,se_rel,spe_rel,pre_rel,f1_rel,loss = evaluate_evi(self.best_model, self.g_valid_pos, self.g_valid_neg, self.G, self.eval_dd_etypes, self.device, mode = 'test', return_raw=False)
        if save_name is not None:
            results = {
                "macro_auroc_test": float(macro_auroc),
                "macro_auprc_test": float(macro_auprc),
                "micro_auroc_test": float(micro_auroc),
                "micro_auprc_test": float(micro_auprc),
                "auroc_rel": {str(k): float(v) for k, v in auroc_rel.items()},
                "auprc_rel": {str(k): float(v) for k, v in auprc_rel.items()},
                "acc_rel": {str(k): float(v) for k, v in acc_rel.items()},
                "se_rel": {str(k): float(v) for k, v in se_rel.items()},
                "spe_rel": {str(k): float(v) for k, v in spe_rel.items()},
                "pre_rel": {str(k): float(v) for k, v in pre_rel.items()},
                "f1_rel": {str(k): float(v) for k, v in f1_rel.items()},
            }
            with open(os.path.join(save_name, "valid_eval.json"), "w") as f:
                json.dump(results, f, indent=2, ensure_ascii=False)
                
        auroc_rel, auprc_rel, micro_auroc, micro_auprc, macro_auroc, macro_auprc,acc_rel,se_rel,spe_rel,pre_rel,f1_rel,loss = evaluate_evi(self.best_model, self.g_test_pos, self.g_test_neg, self.G, self.eval_dd_etypes, self.device, mode = 'test', return_raw=False)
        ...
        if save_name is not None:
            ...
            with open(os.path.join(save_name, "test_eval.json"), "w") as f:
                json.dump(results, f, indent=2, ensure_ascii=False)
    ...
    def evaluation(self,pkl_dir = None,save_path = None):
        etypes_dd = [
        ...
        with torch.no_grad():
            (auroc_rel, auprc_rel, micro_auroc, micro_auprc, macro_auroc, macro_auprc,acc_rel,se_rel,spe_rel,pre_rel,f1_rel), loss, pred_pos, pred_neg = evaluate_test(self.best_model, self.g_test_pos, self.g_test_neg, self.G, etypes_dd, self.device, True, mode = 'test')
        if save_path is not None:
            ...
            with open(os.path.join(save_path, "test_eval.json"), "w") as f:
                json.dump(results, f, indent=2, ensure_ascii=False)
    ...
    def retrieve_sim_diseases(self, relation, k = 5, path = None):
        if relation not in ['CPD_DIS_ass']:
            raise ValueError("Please select the following three relations: 'CPD_DIS_ass' !")
        ...
        if relation == 'CPD_DIS_ass':
            etype = ('disease', 'rev_CPD_DIS_ass', 'compound')               
    ...
    def train_graphmask(self, relation = 'CPD_DIS_ass',
                              learning_rate = 3e-4,
                              allowance = 0.005,
        ...
        for layer in reversed(list(range(self.graphmask_model.count_layers()))):  
            ...
            for epoch in range(epochs_per_layer): 
                ...
                del original_predictions, updated_predictions, f, g, loss, pos_score, neg_score, loss_pred_ori, loss_pred, neg_graph,original_predictions_pos, original_predictions_neg,updated_predictions_pos, updated_predictions_neg
    ...
    def retrieve_gates_scores_penalties(self, relation, no_base = False):
        ...
        # return whole_graph, scores, test_graph
        return scores
                
    def retrieve_save_gates(self, path,relation=None):
        # _, scores, _ = self.retrieve_gates_scores_penalties(relation) 
        scores = self.retrieve_gates_scores_penalties(relation)
