...
class EDAGNN:  
    ...
    def finetune(self, pkl_dir = None,
        ...
        for epoch in range(n_epoch):
            ...
            labels = torch.cat([torch.ones_like(pos_score),torch.zeros_like(neg_score)]).float().to(self.device)
            ...
            if epoch % train_print_per_n == 0:
                # training tracking...
                auroc_rel, auprc_rel, micro_auroc, micro_auprc, macro_auroc, macro_auprc = get_all_metrics_fb(pred_score_pos, pred_score_neg, scores.reshape(-1,).detach().cpu().numpy(), labels.reshape(-1,).detach().cpu().numpy(), self.G, True)
            ...
            if (epoch) % valid_per_n == 0:
                # validation tracking...
                print('Validation.....')
                (auroc_rel, auprc_rel, micro_auroc, micro_auprc, macro_auroc, macro_auprc), loss = evaluate_fb(self.model, self.g_valid_pos, self.g_valid_neg, self.G, self.eval_dd_etypes, self.device, mode = 'valid')
        ...
        print('Testing.......')
        (auroc_rel, auprc_rel, micro_auroc, micro_auprc, macro_auroc, macro_auprc,acc_rel,se_rel,spe_rel,pre_rel,f1_rel), loss, pred_pos, pred_neg = evaluate_test(self.best_model, self.g_valid_pos, self.g_valid_neg, self.G, self.eval_dd_etypes, self.device, True, mode = 'test')
        ...
        (auroc_rel, auprc_rel, micro_auroc, micro_auprc, macro_auroc, macro_auprc,acc_rel,se_rel,spe_rel,pre_rel,f1_rel), loss, pred_pos, pred_neg = evaluate_test(self.best_model, self.g_test_pos, self.g_test_neg, self.G, self.eval_dd_etypes, self.device, True, mode = 'test')
